 ...
 * @param {boolean} [includePaht=true] 默认值true，是否显示基于根路径的完整路径
 * @returns {ArrayStr} 路径数组
 */
function getFile(path: string, includePaht: boolean = true): ArrayStr {
  let urlList: ArrayStr = [];
  let urls: ArrayStr = fs.readdirSync(path);
  urls.forEach(url => {
    const isDir: boolean = fs.statSync(path + '/' + url).isDirectory();
    if (isDir) {
      const list: ArrayStr = getFile(path + '/' + url, includePaht);
      if (includePaht) {
        list.forEach(item => {
